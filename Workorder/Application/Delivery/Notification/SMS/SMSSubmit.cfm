...
<cffile action = "write" file = "#SESSION.rootPath#\CFRStage\user\#SESSION.acc#\Logs\#fname#" attributes = normal output = "SMS tool, #DateFormat(now(),CLIENT.DateFormatShow)# , #TimeFormat(Now(), 'medium')# <br><br>">  
...
<cfoutput>
	...
	<cfloop from="1" to=#ListLen(Form.CSMS)# index="i"> 
	
		<cfset To_Send = "">
		<cfset workorder = ListGetAt(Form.CSMS, i)> 
				...
				<cftry>
						...
						<cfif Find("200",objHttpSession.StatusCode) neq 0>
							...
							<cfif vResultCode eq 10>
								<cfset vResponse = "(#vResultCode#) SMS sent succesfully to #Mobile#">														
...
<script>
	...
	ColdFusion.Window.create('bb_response', 'PROSIS SMS','Notification/SMS/SMSProcessingResponse.cfm?txt=',{height:590,width:350,modal:true,closable:true,draggable:true,resizable:true,center:true,initshow:true })
	ColdFusion.navigate('Notification/SMS/SMSProcessingResponse.cfm?txt=#fname#','bb_response');
	notification('sms');
</script>
</cfoutput>
