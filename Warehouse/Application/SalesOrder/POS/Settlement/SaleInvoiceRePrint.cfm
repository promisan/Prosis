...
<cfif getInvoice.ActionMode eq "2" AND getInvoice.ActionReference3 neq "">
	...
	<tr><td height="100%" style="padding:15px;">
		...
		<cfif getWarehouseJournal.recordCount eq 1 and trim(getWarehouseJournal.TransactionTemplateMode2) neq "">
			<iframe src="#SESSION.root#/#getWarehouseJournal.TransactionTemplateMode2#?batchid=#url.batchid#&terminal=#vTerminal#&source=print&layout=pos"
...
<cfelse>
	...
	<tr><td height="100%" style="padding:15px;">
		...
		<cfif getWarehouseJournal.recordCount eq 1 and trim(getWarehouseJournal.TransactionTemplateMode1) neq "">
			<iframe src="#SESSION.root#/#getWarehouseJournal.TransactionTemplateMode1#?batchid=#url.batchid#&terminal=#url.terminal#&source=print&layout=pos"
			  name="print" id="print" width="100%" height="100%" scrolling="yes" frameborder="0" style="border:1px dashed silver"></iframe>
		<cfelse>
			<table width="100%">
				<tr>
			...
			<td align="center">
				...
				<cf_tl id="Close" var="1">
				 ...
				 <input type  = "button" 
				      ...
				      style   = "height:28;width:150;font-size:13px;border:1px solid silver" 
					  class   = "regular" 
					  name    = "save" id="save"
					  value   = "#lt_text#">
		    </td>
