...
<cfinvoke component = "Service.Process.Materials.Taskorder"  
	...
	deliverydate     = "#url.selecteddate#"
	mode             = "Pending"
	stockorderid     = ""
	selected         = ""
	returnvariable   = "listing">	
		 									
<table width="99%" height="100%" cellspacing="0" cellpadding="0" align="center" class="formpadding">
	...
	<cfoutput>				
	<tr><td colspan="<cfoutput>#col#</cfoutput>" height="50" align="center" class="labelit"><font color="808080">There are no taskorders pending to be processed for #dateformat(url.selecteddate,CLIENT.DateFormatShow)#.</font></td></tr>
	</cfoutput>
	...
	<cfoutput query="Listing" group="shiptomode">
		...
		<b><cf_tl id="#getMode.Description#"></b>
		...
		<cfif TaskorderReference neq "">
			...
			<tr>
			<td colspan="7" style="padding-top:4px;padding-left:8px" class="labelmedium">
		...
		<cfoutput>									
			...
			<tr bgcolor="<cfif recordstatus eq '3'>f1f1f1<cfelseif TaskOrderReference eq ''>f4f4f4</cfif>">									
			<td style="padding-left:8px;padding:2px">
			...
			<td style="padding-left:7px;padding:2px">#ItemDescription#</td>
			<td style="padding:2px">#WarehouseName#</td>
			<td style="padding:2px">#Contact#</td>
			<td style="padding:2px" onclick="window.open('#SESSION.root#/Tools/Mail/MailPrepareOpen.cfm?id=print&ID1=#Reference#&ID0=#Param.RequisitionTemplateMultiple#','_blank', 'left=30, top=30, width=800, height=600, toolbar=no, menubar=no, status=yes, scrollbars=no, resizable=no')">
			<a href="##"><font color="6688aa">#Reference#</a>
			</td>																							
			<td style="padding:2px" colspan="2">#TaskedBy# (#dateformat(TaskedOn,CLIENT.DateFormatShow)#)</td>						
			<td style="padding:2px" id="f#taskid#_shiptomode">
			...
			<td align="right" style="padding:2px">#numberformat(TaskQuantity,'#pformat#')#</td>
			
			<td align="right" id="pending_#taskid#" style="padding:2px" bgcolor="ffffcf">
									 
			     <cfif PickedQuantity gt "0">				 
				 	<cfset pen = TaskQuantity-PickedQuantity>	
				 <cfelse>				 
				 	<cfset pen = TaskQuantity>
				 </cfif>	
			...
			<td style="padding-left:7px;padding:2px">#UoMDescription#</td>	
			<td style="padding-left:4px;padding:2px" id="status#taskid#" align="center">		
				...
				<cfelse>
					 ...
					 <cfelse>
						...
						<cfelse>			
							...
							<!--- we allow for receipt --->
							
							<cfif DeliveryReceipt.ModeShipmentEntry eq "1">
