<cfcomponent
	...
	<cffunction
		...
		returntype="any"
		output="false"
		hint="Returns Xythos STK">
		
		<cfreturn VARIABLES.Instance.STK />
	</cffunction>	
		...
		<cfsavecontent variable="LOCAL.Output">
			<cfoutput>
				...
				<cfif StructKeyExists( ARGUMENTS.Response.ResponseHeader, "Location" )>
					...
					<cfset vStk="#ARGUMENTS.Response.ResponseHeader.Location#">
					<!--- Added by Jorge Mazariegos www.promisan.com --->
					<cfset vPos=Find("stk=",vstk)>
					<cfif vPos neq 0>
						<cfset stk=mid(vStk,vPos+4,len(vStk))>
						 <cfset VARIABLES.Instance.STK=stk>
					</cfif>
				</cfif>
	...
	<cffunction 
		...
		returntype="struct"
		output="false"
		hint="Uses the PUT method to place the next request. Returns the CFHttp response.">
	...
	<cffunction 
		...
		<!--- 
			...
			Jorge Mazariegos, Promisan b.v. Nov 2011
		--->

		<cfif ARGUMENTS.Method neq "put">
				<cfset vRedirect = "no">
		<cfelse>		
				<cfset vRedirect = "yes">				
		</cfif>
			...
			<cfhttpparam type="header" name="host" value="bcs.un.org">
	...
	<cffunction 
		...
		<cfargument
			...
			required="false"
			default=""
			hint="The destination directory" 
			/>	
		...
		<cfargument
			...
			<cfif DDirectory eq "" and DFile eq "">
					<cfhttp
						url="#VARIABLES.Instance.RequestData.Url#"
						method="GET"
						UserName="#VARIABLES.Instance.UserName#"
	...
	<cffunction 
		...
		returntype="struct"
		output="false"
		hint="DELETES a Resource in WebDav">
