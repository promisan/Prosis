...
--->
	...
	<cfset VARIABLES.Instance.RequestData.UserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.127 Safari/537.36" />
...
<!--- Return out. --->
	...
	<cffunction name="getTags"
		...
		<cfargument name="tag"     hint="tag to search"    required="yes">
		<cfargument name="ending"  required="yes">
		...
		<cfloop condition="local.startpos GREATER THAN 0">
			...
			<cfif ending eq 1>
				<cfset local.linkpos = reFindNoCase('<#arguments.tag#\b[^>]*>(.*?)</#arguments.tag#>',arguments.html,local.startpos,'true')>
			<cfelse>
				<cfset local.linkpos = reFindNoCase('<#arguments.tag#\b[^>]*>(.*?)',arguments.html,local.startpos,'true')>
			</cfif>

			<cfif val(local.linkpos.len[1])>

				<cfset local.startpos = local.linkpos.len[1]+local.linkpos.pos[1]>
				<cfset local.this.div_content = trim(mid(arguments.html,local.linkpos.pos[1],local.linkpos.len[1]))>
	...
	<cffunction name="getLinks"
			access="public"
			returntype="array"
			output="yes"
			hint="seperate Links from given HTML string, output as a array">
