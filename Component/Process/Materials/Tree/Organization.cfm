		  ...
		  GROUP BY MissionOrgUnitId	
			...
			<cfcase value="1">
					  ...
					  <cfquery name="Level01" 
						...
						(
							SELECT Count(*)
							FROM Organization O2 INNER JOIN Userquery.dbo.#SESSION.acc##Mission#AssetTree A2
							ON O2.OrgUnit = A2.OrgUnit
							WHERE O2.HierarchyCode like O.HierarchyCode+'%'
							AND O2.Mission     = '#Mission#'
							AND O2.MandateNo   = '#MandateNo#'
							
						) as Counted
					    FROM   #Client.LanPrefix#Organization O LEFT OUTER JOIN Userquery.dbo.#SESSION.acc##Mission#AssetTree A
						ON O.OrgUnit = A.OrgUnit
						...
						<cfloop query="level01">	
							 ...
							 <cfset arrayAppend(result,s)/>									 
						      ...
						      password="#SESSION.dbpw#">
							      ...
							      FROM     #Client.LanPrefix#Organization O INNER JOIN Userquery.dbo.#SESSION.acc##Mission#AssetTree A
										ON A.HierarchyCode LIKE O.HierarchyCode+'%'
