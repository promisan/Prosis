<cfsilent>
	...
	<proDes>DateAdd doesn't work if the time changed due to Daylight Savings Time</proDes>
	<proCom>10-Nov-2008 
			DateAdd doesn't work if the time changed due to Daylight Savings Time starting 
			or ending. Adding only adds the number of hours, when the time changes another 
			hour may be added or removed. Using CreateDateTime instead, ignores that. </proCom>
</cfsilent>
...
<cftransaction> 
	...
	<cfset dateValue = "">
		...
		<cfelse>
		  ...
		  --->
		  <!--- MKM: 10-Nov-2008
		  	DateAdd doesn't work if the time changed due to Daylight Savings Time starts 
			or end. Adding only adds the number of hours, when the time changes another 
			hour may or may not be added or removed. Using CreateDateTime instead, ignores that. 
		  <cfset dep = DateAdd("h", "#Form.DepartureHour#",   "#DEP#")> 
		  <cfset dep = DateAdd("n", "#Form.DepartureMinute#", "#dep#")> --->
		  <cfset Departure = #CreateDateTime(Year(dep), Month(dep), Day(dep), #Form.DepartureHour#, #Form.DepartureMinute#, 0)#>
		  <cfset dep = "#Departure#">
		  ...
		  <!--- MKM: 10-Nov-2008
			...
			--->		
			<!--- FDT: 10-Nov-2008 I believe that here it is arr (arrival date) and not dep which should be used 
		  				<cfset Arrival = #CreateDateTime(Year(dep), Month(dep), Day(dep), #hour#, #min#, #sec#)#>
			--->
		  				<cfset Arrival = #CreateDateTime(Year(ARR), Month(ARR), Day(ARR), #hour#, #min#, #sec#)#>
			
<!--- 
Add thirty seconds, in case departure time equals arrival time 
This should rarely happen, but it may if the International Date Line was crossed during the trip 
--->
						<cfset Arrival = DateAdd("s", "30", "#Arrival#")> 		 
						<cfset arr = "#Arrival#">
		  ...
		  <!--- MKM: 10-Nov-2008
			...
			hour may or may not be added or removed. Using CreateDateTime instead, ignores that. 
							  ...
							  <cfset Departure = #CreateDateTime(Year(dep), Month(dep), Day(dep), #hour2#, #min2#, 0)#>
							  <cfset dep = "#Departure#">						  
...
we'll need to really fix this code. --->
	...
	<cfquery name="EventPerson" 
	 ...
	 password="#SESSION.dbpw#">
		 ...
		 FROM   ClaimRequest C, stPerson P 
