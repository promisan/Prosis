...
<script language="JavaScript">
 ...
 function savebox(dte,per,cat,ind,val) {
      ColdFusion.navigate('ClaimEventEntryDSASave.cfm?claimid=<cfoutput>#url.claimid#</cfoutput>&date='+dte+'&personno='+per+'&cat='+cat+'&ind='+ind+'&val='+val,'result')
 }
...
<table><tr class="hide"><td><cfdiv id="result"></td></tr></table>
...
<!--- MKM: 26-November-2008
The following "if" was copied from ClaimPreparation.cfm. 
It is normally called later on from this template, but we need the information now
on whether or not the claim can be edited.
--->
...
<!--- FDT/MKM 25/11/2008 Tp prevent the lauching of CalculateDSA and the reinitialization of ClaimLineDSA 
on Viewing by EO/Accounts of the Subsistence Screen ---> 
<cfif EditClaim eq 1>  
	...
	<cfif Period.Expiration lte Period.Start>
		...
		<cfelse>
			...
			<cf_ClaimEventEntryIndicatorPointer
			 ...
			 value      = "checked"
			 line       = "1"
			 formsubmit = "ClaimEventEntryDSASubmit.cfm?Section=#URL.Section#&ClaimId=#URL.ClaimId#&ID1=#URL.ID1#&Category=period"
			 formname   = "entry"			 
...
<!---	MKM: 12-Nov-2008				This is incorrect.								
	The query always removes entries for the first travel day. 
