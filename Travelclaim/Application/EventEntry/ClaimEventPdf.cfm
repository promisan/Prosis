...
<table width="100%" height="100%" cellspacing="0" cellpadding="0" align="center">
   ...
   --->
			...
			<cfif currentrow eq "1" or url.topic neq "trip">
				...
				<cfloop query="trip">
				   ...
				   <tr>
							...
							<cfinclude template="ClaimDeleteHired.cfm">
							</cfif>
	...
	<!--- JG this is the place I would be interested in adding  to print Summary
	
	<cfquery name="Claimpdf" 
									datasource="appsTravelClaim" 
									username="#SESSION.login#" 
								password="#SESSION.dbpw#">
						SELECT CI.InvoiceCurrency, CE.IndicatorCode,sum(CI.InvoiceAmount) as TotAmount
 FROM ClaimEventIndicatorCost CI , ClaimeventIndicator CE
where CI.claimeventid = CE.claimeventid
and CI.claimeventid in (select claimeventid from claimevent
where claimid ='#URL.Claimid#')
group by CI.InvoiceCurrency,CE.IndicatorCode
	 ...
	 <!--- JG added --->
								<cfquery name="Claimpdf" 
									...
									username="#SESSION.login#" 
								password="#SESSION.dbpw#">
						SELECT CL.claimcategory,sum(CL.amountclaim) as TotAmount,CL.currency as Currency_line, CH.Paymentcurrency as Header_currency
						from claimline CL ,claim CH
where CH.claimid ='#URL.Claimid#'
and CH.claimid =CL.claimid
group by CL.claimcategory,CL.currency,CH.Paymentcurrency

	</cfquery>	
	 <cfset claim_currency =Claimpdf.header_currency>
	<cfquery  name="ClaimPdfSummary" dbtype="query"  >
						SELECT sum(TotAmount) as SumAmount
						from claimpdf
						 
	</cfquery>	
	<cfquery  name="ClaimMSCSummary" dbtype="query"  >
						SELECT sum(TotAmount) as SumAmount
						from claimpdf
						where claimcategory not in ('DSA','TRM')
						 
	</cfquery>	
									
    <cfloop query="claimpdf" >
		<cfif claimcategory eq "SFT" >
			<cfset Title_msc ="Other Expenses/Self Ticket Total:">
			<cfbreak>
		<cfelse>
			<cfset Title_msc ="Other Expenses Total:">
			</cfif>
	</cfloop>
									 <!--- JG added --->
	
	<cfoutput query="ClaimPdf" > 
	<cfset claim_header_currency =Claimpdf.Header_currency>
	<cfset space_neat ="  ">
	<cfif  ClaimPdf.claimcategory eq "TRM" >
	
	
		<tr bgcolor="f5f5f5">
		        
				<td colspan="8"><b>Itinerary Total:</b></td>
				<td align="right">#claim_header_currency# #space_neat# #numberformat(ClaimPdf.TotAmount,"__,__.__")#</td>&nbsp;
				<!---
				<td>&nbsp;</td>
				&nbsp;<td>#numberformat(ClaimPdf.TotAmount,"__,__.__")#</td>
				--->
	...
	<cfquery  name="ClaimEventPeriodDSASummary" dbtype="query"  >
						SELECT sum(converted_amt) as DSASumAmount
						from Period
						
						 
	</cfquery>	
	
		<cfoutput query="ClaimEventPeriodDSASummary">
		<tr bgcolor="f5f5f5">
		       	<td colspan="8"><b>Subsistence Details Total:</b></td>
				<td align="right">#claim_header_currency# #space_neat# #numberformat(ClaimEventPeriodDSASummary.DSASumAmount,"__,__.__")#</td>
		...
		<cfif claim.claimAsIs eq "0">
			<cfset cls = "OTH">
			<!-- JG Added it for other but this throws off other stuff completely
			
			<cfquery name="DetailsOTH" 
			datasource="appsTravelClaim" 
			username="#SESSION.login#" 
			password="#SESSION.dbpw#">
			...
			AND      I.ClaimCategory = 'MSC'
			ORDER BY InvoiceDate
			</cfquery>
			 
			<cfloop query="DetailsOTH">
					<tr bgcolor="ffffdf">
						<td>#currentRow#.</td>
	...
	<cfoutput query="ClaimMSCSummary" > 
	 
	
		<tr bgcolor="f5f5f5">
		        
				<td colspan="8"><b>#Title_msc#</b></td>
				
				<td colspan="2" align="right">#claim_header_currency#  #numberformat(ClaimMSCSummary.SumAmount,"__,__.__")# </td>
	...
	<!---  End of JG adding --->					
		...
		<!---  Start of JG adding --->
		<cfinclude template ="Summarypdf.cfm">
		<!---  End of JG adding --->
