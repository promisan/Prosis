...
<cf_mobile appId="Hub" chartJS="yes" validateLogin="no" allowLogout="no" grid="yes">
	...
	<script>
		...
		function applyFilter(template, sd, container) {
			ColdFusion.navigate(template+'?showdivision='+sd+'&pMission='+$('#pMission').val()+'&pFund='+$('#pFund').val()+'&pYear='+$('#pYear').val()+'&pDonor='+$('#pDonor').val()+'&pPeriod='+$('#pPeriod').val()+'&sortfield='+$('#pSortField').val()+'&sortdirection='+$('#pSortDirection').val(), container);
		}		
		
		function doSorting(fd, template, sd, container) {
			$('#pSortField').val(fd);
			if ($('#pSortDirection').val() == 'ASC') {
				$('#pSortDirection').val('DESC');
			} else {
				$('#pSortDirection').val('ASC');
			}
			applyFilter(template, sd, container);
		}
		
		function clickMap(e){
			var vContextValue = encodeURIComponent(e.mapObject.id);
			ColdFusion.navigate('Collection/SummaryTable.cfm?showdivision=0&pMission='+$('#pMission').val()+'&pFund='+$('#pFund').val()+'&pYear='+$('#pYear').val()+'&pDonor='+$('#pDonor').val()+'&pPeriod='+$('#pPeriod').val()+'&sortfield='+$('#pSortField').val()+'&sortdirection='+$('#pSortDirection').val()+'&country='+vContextValue, 'divCollectionTable', function(){
				$('html, body').animate({
                    scrollTop: $('#divCollectionTable').offset().top
                }, 500);
			});
		}
	...
	<div id="rowDetail" style="display:none"></div>	
		...
		<cf_mobileLayoutArea position="left" brandlogo="img/Financials.png">
			...
			<cf_mobileMenu>
				...
				<cf_tl id="Core Trust Fund" var="1">
				...
				<cf_tl id="Voluntary Trust Fund" var="1">
