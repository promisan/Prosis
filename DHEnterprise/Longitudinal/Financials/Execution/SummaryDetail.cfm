...
<cfquery name="getData" 
	...
	<!--- Hanno to be aadjusted
		
		AND        LEFT(CBWBSe, 12) IN (
		
					SELECT    PeS.SourceNo
					FROM      NYVM1613.Program.dbo.ProgramPeriodSource AS PeS INNER JOIN
			                  NYVM1613.Program.dbo.ProgramPeriod AS Pe ON PeS.ProgramCode = Pe.ProgramCode AND PeS.Period = Pe.Period INNER JOIN
			                  NYVM1613.Program.dbo.Program AS P ON PeS.ProgramCode = P.ProgramCode
					WHERE     Pe.RecordStatus = '1' and Status != '9'
					AND       P.Mission = '#url.pmission#'
