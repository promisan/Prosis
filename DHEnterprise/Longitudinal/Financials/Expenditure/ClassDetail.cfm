...
<!--- point this query to the final table : posting class --->
...
<cfquery name="getBaseData" 
	datasource="HubEnterprise">
		...
		FROM       FinanceTransaction AS B INNER JOIN
		           Ref_GLAccount AS R ON B.GLAccount = R.GLAccount INNER JOIN 
				   Ref_ObjectClass O ON R.SponsoredClass = O.ObjectClass
		WHERE      B.Mission  = '#url.pmission#'
		AND        B.CBPeriod = '#url.pPeriod#' 
		AND        B.PostingClass = 'Execution' 
