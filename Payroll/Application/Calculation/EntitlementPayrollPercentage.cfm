...
<cfloop query="setschedule">   
	...
	<cfloop query = "base">
		...
		password="#SESSION.dbpw#">
			...
			<!--- 18-12-2018 added by Dev, to be clarified as to why we needed this code --->
		</cfquery>
		...
		<cfloop index="itm" list="0,1,2,3" delimiters=",">
			...
			<cfquery name="CalculatePayrollAmountPercentage" 
				...
				<!--- 	Dev note : this variation was introduced if a percentage would beed needed for a dependent which entitlement start
				...
				SET   	EntitlementAmountFull = ((Percentage/100)*AmountCalculationFull)*SalaryMultiplier, 
						...
						END,										
						Dev 20/2 adjusted the = '1' to take the full amount as recorded AmountCalculationFull. We had an issue with a person that had only partial period of allowance
						--->
