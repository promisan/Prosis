...
password="#SESSION.dbpw#">
	...
	AND A.Role NOT IN (SELECT Role 
	                     FROM Payroll.dbo.Ref_PayrollItemRole 
...
password="#SESSION.dbpw#">
    SELECT R.*
    FROM   Ref_PayrollItemRole R
	WHERE  R.Operational = 1
	AND    PayrollItem = '#URL.ID#'
</cfquery>
...
<cfform action="#SESSION.root#/Payroll/Maintenance/SalaryItem/ItemRoleSubmit.cfm?ID=#URL.ID#&ID1=#URL.ID1#" method="POST" name="role">
	
<table width="96%" height="100%" align="center" class="navigation_table">
	  ...
	  <tr>
							...
							<td align="right">#OfficerUserId#&nbsp;(#dateformat(created,CLIENT.DateFormatShow)#)</td>				 
						   	<td style="padding-left:7px;padding-right:3px">
								<table cellspacing="0" cellpadding="0">
									<tr>
										<td>
										<cf_img icon="open" onclick="ptoken.navigate('#SESSION.root#/Payroll/Maintenance/SalaryItem/ItemRole.cfm?ID=#URL.ID#&ID1=#role#','contentbox1'')">
										</td>
										<td style="padding-left:5px;">
										<cf_img icon="delete" onclick="ptoken.navigate('#SESSION.root#/Payroll/Maintenance/SalaryItem/ItemRolePurge.cfm?ID=#URL.ID#&ID1=#role#','contentbox1')">
										</td>
									</tr>
								</table>
							</td>
		...
		<cfif URL.ID1 eq "">
		   ...
		   <cfselect name="Role"
				  ...
				  class="regularxxl"
		          enabled="Yes"
		          onchange="ptoken.navigate('#SESSION.root#/Payroll/Maintenance/SalaryItem/ItemRoleAdd.cfm?role='+this.value,'roleadd')"
		          style="width:340px;height:32px;font-size:16px">
		   ...
		   <td id="roleadd" style="padding-left:8px;fixlength"></td>
		...
		<tr><td height="2" colspan="5" class="labelmedium"><b><font color="FF0000">Attention:</font></b> No active roles defined. This item is not restricted by roles</td></tr>		
		</cfif>
