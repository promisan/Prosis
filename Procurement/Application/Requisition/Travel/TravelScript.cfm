...
<script>
  ...
  function changeDate() {
	...
	function itinvalidate() {
		...
		if( _CF_error_messages.length == 0 ) {	
	        ptoken.navigate('#SESSION.root#/Procurement/Application/Requisition/Travel/ItinerarySubmit.cfm?scope=requisition&pers='+per+'&ID=#URL.ID#','iservice','','','POST','itinedit')
		 }   
	}
