<!--- 
	DeploymentListing.CFM

	View all UNMO/CIVPOL personnel that are/were deployed in a field mission.
	
	Calls:
	DeleteAssigment.cfm
	DeleteAssigmentAndPerson.cfm
	
	Modification History:
	031001 - created by MM
	040404 - changed the Category droplist to filter via PersonCategory rather than post type
	040607 - added code to prevent listing of Person twice as a result of a person being a "DocumentCandidate" on both
		   - an active and an inactive "Document"
	041006 - added LEFT JOIN to DocumentNo to prevent list of Assignment Record twice as a result of having multiple replacements 
	...
	050108 - added new button to delete both Assignment and Person records.  Also new function DeleteAssignmentAndPerson()
...
--->
<HTML><HEAD><TITLE>Deployed Personnel</TITLE></HEAD>
...
function reloadForm(nat,group,page,mission,categ,crit) {
    window.location="DeploymentListing.cfm?IDNationality=" + nat + "&IDCategory=" + categ + "&IDSorting=" + group + "&Page=" + page + "&IDMission=" + mission + "&IDCriteria=" + crit;
}

function DeleteAssignment(ass) {
	if (confirm("Do you want to delete this assignment record?")) {
		window.open("DeleteAssignmentSubmit.cfm?ID=" + ass, "actionsubmit", "width=200, height=200, toolbar=no, scrollbars=no, resizable=yes");
	}
}

function DeleteAssignmentAndPerson(ass, per) {
	if (confirm("Do you want to delete this assignment and person record?")) {
		window.open("DeleteAssignmentAndPersonSubmit.cfm?ID=" + ass + "&ID2=" + per, "actionsubmit", "width=200, height=200, toolbar=no, scrollbars=no, resizable=yes");
	}
}
</script>	
...
<cfif ParameterExists(Form.DepartureFrom)>
	<cfset dateValue = "">
	<cfif #Form.DepartureFrom# NEQ "">
 		<CF_DateConvert Value="#Form.DepartureFrom#">
		<cfset d1Date = #dateValue#>
		<cfif #Criteria# EQ ""><cfset #Criteria# = "PA._tsDateActualDeparture >= #d1Date#">
		<cfelse><cfset #Criteria# = #Criteria#&" AND PA._tsDateActualDeparture >= #d1Date#">
		</cfif>
	</cfif>
</cfif>
<cfif ParameterExists(Form.DepartureTo)>
	<cfset dateValue = "">
	<cfif #Form.DepartureTo# NEQ "">
 		<CF_DateConvert Value="#Form.DepartureTo#">
		<cfset d2Date = #dateValue#>
		<cfif #Criteria# EQ ""><cfset #Criteria# = "PA._tsDateActualDeparture <= #d2Date#">
		<cfelse><cfset #Criteria# = #Criteria#&" AND PA._tsDateActualDeparture <= #d2Date#">
		</cfif>
	</cfif>
</cfif> 
...
<cfquery name="MaxAssignmentNo" datasource="AppsEmployee" 
 username="#SESSION.login#" password="#SESSION.dbpw#">
	...
	FROM PersonAssignment A INNER JOIN 
		...
		userQuery.dbo.tmp#SESSION.acc#pm_SelectedCateg SC ON P.Category = SC.Category 	
	GROUP BY A.PersonNo
</cfquery>
...
SELECT PO.Mission, P.PersonNo, R.ShortDesc AS Rank, N.Name as CountryName,
	...
	RPD.MaxDocNo AS sDocNo, DC.FirstName + ' ' + DC.LastName AS Replacement, DC.PlannedDeployment
FROM  
	...
	DocumentCandidate DC ON DRR.ReplacementPersonNo = DC.PersonNo AND RPD.MaxDocNo = DC.DocumentNo  LEFT JOIN 
...
<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#002350" frame="all">
  <tr bgcolor="#002350" height="20" valign="middle">
    <td colspan="4"><font face="Tahoma" size="2" color="#FFFFFF"><b>&nbsp;DEPLOYED PERSONNEL</b></font>&nbsp;&nbsp;-&nbsp;&nbsp;<font face="Tahoma" size="1" color="#FFFFFF">
	This view lists all personnel who have been deployed in a field mission.  An * after the LastName indicate those deployed via PM STARS.</td>
  </tr>
...
<!---	
  		...
  		<img src="../../Images/print.jpg" alt="Print" width="25" height="21" border="0" onClick="">
		<font face="Tahoma" size="1" color="#FFFFFF"><b>Print<b></b></font></a>&nbsp;		
--->		
	</td>
</tr> 	
<!-- End RECORD FILTERS -->
...
<table width="100%" border="1" cellspacing="0" cellpadding="0" align="center" bordercolor="#6688aa" rules="cols" style="border-collapse: collapse">
   <tr bgcolor="#6688AA" height="30" valign="middle">
	...
	onChange="javascript:reloadForm(nat.value,this.value,1,mission.value,categ.value,criter.value)">
	<cfif #nationality.recordcount# GT 1>
...
<tr>
	...
	<td width="4%"><font size="-2" face="Arial, Helvetica, sans-serif">Rank</font></td>
	...
	<td width="8%"><font size="-2" face="Arial, Helvetica, sans-serif">Birth Date</font></td>
	<td width="5%"><font size="-2" face="Arial, Helvetica, sans-serif">Cat</font></td>
	<td width="10%"><font size="-2" face="Arial, Helvetica, sans-serif">Functional Title</font></td>
	<td width="3%"><font size="-2" face="Arial, Helvetica, sans-serif">TOD</font></td>
    <td width="8%"><font size="-2" face="Arial, Helvetica, sans-serif">Arrival</font></td>
	<td width="8%"><font size="-2" face="Arial, Helvetica, sans-serif">Rotation</font></td>
	<td width="7%"><font size="-2" face="Arial, Helvetica, sans-serif">Departure</font></td>	
    <td width="10%"><font size="-2" face="Arial, Helvetica, sans-serif">Replacement</font></td>
	<cfif #SESSION.acc# EQ "dpknyjs77" OR #SESSION.acc# EQ "dpknycp2" OR #SESSION.acc# EQ "dpknyfs1" OR #SESSION.acc# EQ "fodnyma4">	
		<td width="6%"><font size="-2" face="Arial, Helvetica, sans-serif">Delete</font></td>
	<cfelse>
...
<tr bgcolor="C0C0C0"><td height="1" colspan="16" class="top2"></td></tr>
...
<td class="regular"><a href="javascript:pm_editperson('#PersonNo#')" title="Click to view/edit person record. An asterisk on Last Name indicates person travelled via PM STARS.">#FirstName#</a></td>
<td class="regular"><a href="javascript:pm_editperson('#PersonNo#')" title="Click to view/edit person record. An asterisk on Last Name indicates person travelled via PM STARS.">#LastName#</a></td>
<td class="regular">#DateFormat(BirthDate, "#CLIENT.dateformatshow#")#</td>
<td class="regular">#Category#</td>
<td class="regular">#FunctionDesc#</td>
<td class="regular">#TOD#</td>
<td class="regular"><a href="javascript:pm_editassigndates('#AssignmentNo#')" title="Click to view/edit assignment details.">#DateFormat(DateArrival, "#CLIENT.dateformatshow#")#</a></td>
<td class="regular"><a href="javascript:pm_editassigndates('#AssignmentNo#')" title="Click to view/edit assignment details.">#DateFormat(DateDeparture, "#CLIENT.dateformatshow#")#</a></td>
<td class="regular"><a href="javascript:pm_editassigndates('#AssignmentNo#')" title="Click to view/edit assignment details.">#DateFormat(ActualDeparture, "#CLIENT.dateformatshow#")#</a></td>

<td class="regular">#Replacement#</td>
<cfif #SESSION.acc# EQ "dpknyjs77" OR #SESSION.acc# EQ "dpknycp2" OR #SESSION.acc# EQ "dpknyfs1" OR #SESSION.acc# EQ "fodnyma4">	
	<td class="regular">&nbsp;
	    <a href ="javascript:DeleteAssignment('#AssignmentNo#')">
    	     <img src="#CLIENT.Root#/Images/button.jpg" alt="" name="img0_#AssignmentNo#"  width="14" height="14" border="0" align="middle">
        </a>&nbsp;
	    <a href ="javascript:DeleteAssignmentAndPerson('#AssignmentNo#','#PersonNo#')">
    	     <img src="#CLIENT.Root#/Images/buttonred.jpg" alt="" name="img0_#AssignmentNo#"  width="14" height="14" border="0" align="middle">
        </a>&nbsp;
	</td>
<cfelse>
...
<tr><td height="10" colspan="16"></td></tr>
</cfif>
</cfoutput>
...
<td align="right"><font size="1" face="Tahoma"><b><cfoutput>#NumberFormat(AmtT,'_____,__')#&nbsp;</cfoutput></b></font></td>	
	...
	<tr><td height="10" colspan="16" bgcolor="#002350"></td></tr>		<!--- Print a dark blue border --->
</table>
