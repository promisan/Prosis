<cfif #Form.undoActionId# eq ''>

<cfoutput>

<cfloop index="Rec" from="#CLIENT.RecordNo#" to="1" step="-1">
  ...
  <!--- STATUS : capture values in an array --->
   ...
   <cfquery name="Update" 
	...
	<!--- verify if it is an XOR action that was completed --->
	...
	AND  ActionId        = '#actionid#'
	  ...
	  <cfif NA.ActionForm neq "">
	
	<cfquery name="NAUpdateR" 
    datasource="#CLIENT.Datasource#" 
    username="#SESSION.login#" 
    password="#SESSION.dbpw#">
	UPDATE DocumentCandidateAction 
	...
	AND  F.DocumentNo      = D.DocumentNo
	AND  F.ActionId        = D.ActionId
	AND  F.ActionForm      = '#NA.ActionForm#'
	 </cfquery>
 ...
 <cfoutput>
  ...
  <cfif #WhichModule.Name# EQ "vacancy">
	  <cflocation url="#CLIENT.root#Vacancy/Application/DocumentCandidateEdit.cfm?ID=#Form.DocumentNo#&ID1=#Form.PersonNo#&IDArea=#Form.AreaSelect#"> 
  <cfelse>
	  <cflocation url="../Application/DocumentCandidateEdit.cfm?ID=#Form.DocumentNo#&ID1=#Form.PersonNo#&IDArea=#Form.AreaSelect#">
  </cfif>