...
<CFELSE>   
   ...
   <cfquery name="GetMissionInfo" datasource="#CLIENT.Datasource#" username="#SESSION.login#" password="#SESSION.dbpw#">   
    SELECT RM.Mission, RM.MandateNo, RM.DateEffective, RM.DateExpiration, C.Category, C.TravellerType, 
		OrgUnit = (CASE WHEN C.TravellerType = 'MILITARY' THEN RM.MilitaryOrgUnit ELSE RM.CivpolOrgUnit END)
	  FROM Document D, Ref_TVL_Mandate RM, Ref_Category C
	  WHERE D.Mission = RM.Mission
	  AND   D.PersonCategory = C.Category
	  AND   D.DocumentNo = #Attributes.Action#
   </cfquery>
