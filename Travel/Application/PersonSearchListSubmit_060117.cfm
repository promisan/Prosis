...
<!---
SELECT     DRR.*
FROM       DocumentRotatingPerson DRR
WHERE      PersonNo = '#URL.ID1#' 
AND EXISTS
	(SELECT *
     FROM   DocumentCandidate DC, Document D
     WHERE  DC.DocumentNo = D.DocumentNo 
	 AND 	D.Status = '0' 
	 AND 	DC.Status NOT IN ('6', '9') 
	 AND	DC.PersonNo = DRR.ReplacementPersonNo)
