<!---
	...
	02Dec04 - modified qPerson query to add this lines:
			...DocumentCandidate DC ON DRP.DocumentNo = DC.DocumentNo AND DRP.ReplacementPersonNo = DC.PersonNo LEFT JOIN
			...EMPLOYEE.DBO.Person P2 ON DC.PersonNo = P2.PersonNo LEFT JOIN...
			- also modified following line:
			...AND CONVERT(Char(10), PA.DateArrival, 20) + CONVERT(Char(10), PA.DateDeparture, 20) = MPA.MaxAssignmentDate) LEFT JOIN ...
			to
			...AND CONVERT(Char(10), PA.DateArrival, 20) + CONVERT(Char(10), PA.DateDeparture, 20) + CONVERT(Char(10), PA.Created, 20)= MPA.MaxAssignmentDate) LEFT JOIN ...			
			- this was needed to prevent the name of the replacement person (which may be stalled) from appearing
			  in the last column in the cases of rotating persons that are auto-copied onto a new
			  request record.
			- the addition of PA.Created in the JOIN required modification of the vwMaxPersonAssignment view
	15Oct12 - View vwMaxPersonAssignment replaced in main Query in order to pull the last assignment that is
	          effective on or before the Planned Deployment date of the current deployment request
--->
...
<!--- this query uses TRAVEL.DBO.vwMaxPersonAssignment to ensure that only the last assignment is joined to the
	  ...
	  15Oct2012 - View vwMaxPersonAssignment replaced by embedded query below in order to pull the last assignment
	              effective on or before the Planned Deployment date of the current deployment request
	  		(
			SELECT     TOP (100) PERCENT PersonNo, MAX(CONVERT(Char(10), DateArrival, 20) + CONVERT(Char(10), DateDeparture, 20) + CONVERT(Char(10), Created, 20)) 
    	                  AS MaxAssignmentDate
			FROM         Employee.dbo.PersonAssignment
			WHERE       DateEffective <= (SELECT PlannedDeployment FROM Document WHERE DocumentNo = '#URL.ID#')
			GROUP BY PersonNo
			ORDER BY PersonNo
		   )
--->
<cfquery name="qPerson" datasource="#CLIENT.Datasource#" username="#SESSION.login#" password="#SESSION.dbpw#">
...
FROM   	DocumentRotatingPerson DRP INNER JOIN 
		...
		(
			...
			GROUP BY PersonNo
			ORDER BY PersonNo
		) AS MPA ON P.PersonNo = MPA.PersonNo LEFT JOIN 
		EMPLOYEE.DBO.PersonAssignment PA 
		 ON (PA.PersonNo = MPA.PersonNo
		 AND CONVERT(Char(10), PA.DateArrival, 20) + CONVERT(Char(10), PA.DateDeparture, 20) + CONVERT(Char(10), PA.Created, 20)= MPA.MaxAssignmentDate) LEFT JOIN 
