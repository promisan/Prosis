...
<!--- 
	...
	Revision History:
	02Feb04 - added code to handle new Show Filter (Processed by:) -- TU staff who processed request
	10May04 - added new column TA#(s); adjusted other columns as necessary
	19Jun04 - added code to show icon before records where the date of submission
	          to Travel Unit is less than 16 days of the expected deployment date
			- also removed Created On column
	29Jul04 - added code:
			  - to switch layout between Listing and Detail views
			  - added new column Expected Deployment
	          - to allow filter by LastName and DocNo
			  - note these mods required changes to spActionListing and spActionListingALLMISSIONS as well as new view vwCandidateExpectedDeployment
    09Sep04 - modified code to default to DETAIL layout
	11Sep04 - adjusted P_Mission and Mission drop list to take into consideration current value of Status and Workflow Class filters,
		    - as well as Personnel Categories current user has permissions for.
	21sep07 - adjusted references to "../../images/" to "../../../images"
			- changed input button to use class="input.button1" and class="input.button3"			
--->

<cf_screentop html="Yes" jQuery="Yes" banner="red" layout="webapp" scroll="Yes" label="Personnel requests">
...
function reloadForm(docno,olastname,ilastname,perm,group,page,mission,st2,fill,tustaff,area3,lo) {
    window.location="DocumentListing.cfm?IDDocumentNo=" + docno + 
...
<FORM name="result" id="result" style="height:100%">

<table width="100%" style="padding:10px;height:100%" border="0" cellspacing="0" cellpadding="0">

 <tr><td height="7"></td></tr>
 
 <tr><td colspan="10" height="20">
   
 <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
 
 <tr height="25" valign="middle">
	<td height="30" class="labelit">PM:</td>
	<td>
    <select name="p_mission" class="regularxl"  accesskey="P" title="Permanent Mission Selection" 
    ...
    <td class="labelit" style="padding-left:10px">Field Miss:</td>
	<td class="labelit" style="padding-left:10px">	
	<!--- dropdown to select a different mission --->
	<cfif CanAccessAllMissions>
	    <select name="mission" class="regularxl" accesskey="P" title="Field Mission Selection" 
    ...
    <td class="labelit" style="padding-left:10px">Stat:</td>
	<td class="labelit" style="padding-left:10px">	
	<!--- drop down to filter on a document status --->
	<select name="stat2" class="regularxl" accesskey="P" title="Status Selection" 
    ...
    <td class="labelit" align="right" style="padding-left:10px">Workflow:</font></td>
	<td class="labelit" align="right" style="padding-left:10px">
	<!--- drop down to select a action class list --->	
	<select name="fill" class="regularxl" accesskey="P" title="Action Class Selection" 
	onChange="javascript:reloadForm(doc.value,olast.value,ilast.value,p_mission.value,group.value,page.value,mission.value,stat2.value,this.value,tustaff.value,area2.value,lay.value)">
    	<option value="" <cfif URL.IDClass eq "">selected</cfif>>All</option>
	    <cfoutput query="Class">
		<option value="#ActionClass#" <cfif ActionClass is URL.IDClass>selected</cfif>>#Description#</option>
		</cfoutput>
	</select>
	
	</td>
	
    <td align="right" style="padding-left:10px" class="labelit">Proc by:</font></td>
	<td align="right" style="padding-left:10px">
	<!--- drop down to select TU staff who processed/is processing this request --->	
	<select name="tustaff" class="regularxl" accesskey="P" title="Processed by" 
  ...
  <tr>
   ...
   <tr height="25" valign="middle">
	<td class="labelit">Group by:</td>
	<td style="padding-left:10px">
		<!--- drop down to order the screen list --->		
		<select name="group" size="1" class="regularxl"
   	...
   	<td align="right" class="labelit">Show records for:
	<select name="area2" size="1" class="regularxl"
   ...
   <tr height="20" valign="middle">
	...
	<td colspan="2" style="padding-left:10px" class="labelit">Rotating Person:</td>
	<td>

    <input type="text" name="olast" value="<cfoutput>#URL.IDoLastName#</cfoutput>" size="15" class="regularxl" 
	onClick="javascript:clearname()" onKeyUp="javascript:search()">
	</td>
	<td colspan="2" class="labelit">Nominee:</td>
	<td>
    <input type="text" name="ilast" value="<cfoutput>#URL.IDiLastName#</cfoutput>" size="15" class="regularxl" 
	onClick="javascript:clearname()" onKeyUp="javascript:search()">
	</td>
	<td colspan="2" class="labelit">Req No:</td>
	<td>
   	<input class="regularxl" type="text" name="doc" value="<cfoutput>#URL.IDDocumentNo#</cfoutput>" size="4" 
	 onClick="javascript:clearno()" onKeyUp="javascript:search()">
    
	<input type="button" name="submitgo" value="go" class="button10s" style="height:25px"    
	...
	<td align="right" class="regularxl">&nbsp;Layout:</td>
	<td>
   ...
   <tr>
	...
	<tr>
		<td width="4%"  class="labelit"></td>
		<td width="6%"  class="labelit">Req #</font></td>
	    <td width="17%" class="labelit">Field Mission / Stat</td>
		<td width="11%" class="labelit">Perm Mission</td>
		<td width="6%"  class="labelit">Pers Typ</td>	
		<td width="9%"  class="labelit">Req Date</td>
		<td width="6%"  class="labelit">Planned Deployment</td>	
		<td width="2%"  class="labelit">&nbsp;</td>
		<td width="8%"  class="labelit">Expected Deployment</td>
	    <td width="3%"  class="labelit"># of pers</td>
		<td width="4%"  class="labelit">TOD</td>
	    <td width="9%"  class="labelit">TA#(s)</td>
	    <td width="15%" class="labelit">Created by</td>
	</tr>
	...
	<tr bgcolor="f6f6f6">
   	<cfswitch expression = #URL.IDSorting#>
	     <cfcase value = "Created">
		     <td colspan="13" class="labelmedium" bgcolor="##FFFFFF"><b>&nbsp;#Dateformat(Created, "#CLIENT.dateformatshow#")#</b></font></td>
    	 </cfcase>
	     <cfcase value = "PlannedDeployment">
		     <td colspan="13"  class="labelmedium"><b>&nbsp;#Dateformat(PlannedDeployment, "#CLIENT.dateformatshow#")#</b></font></td>
    	 </cfcase>
	     <cfcase value = "ExpectedDeployment">
		     <td colspan="13" class="labelmedium"><b>&nbsp;#Dateformat(ExpectedDeployment, "#CLIENT.dateformatshow#")#</b></font></td>
    	 </cfcase>		 
	     <cfcase value = "RequestDate">
		     <td colspan="13" class="labelmedium"><b>&nbsp;#Dateformat(Requestdate, "#CLIENT.dateformatshow#")#</b></font></td>
    	 </cfcase>
	     <cfcase value = "DutyLength">
			 <td colspan="13" class="labelmedium"><b>&nbsp;#DutyLength#</b></font></td>
    	 </cfcase>
	     <cfcase value = "Mission">
      		 <td colspan="13" class="labelmedium"><b>&nbsp;#Mission#</b></font></td> 
     	</cfcase>	
     	<cfcase value = "Description">
     		 <td colspan="13" class="labelmedium"><b>&nbsp;#Description#</b></font></td> 
     	</cfcase>	
	     <cfcase value = "ActionDescription">
		     <td colspan="13" class="labelmedium" >
	...
	<cfelse>
		...
		<cfif PersonNo eq "0">
			...
			<tr bgcolor="#IIf(CurrentRow Mod 2, DE('ffffff'), DE('EBEBEB'))#">
				...
				<td align="center" valign="middle" style="padding-top:1px">
				    ...
				    <cf_img icon="edit" onclick="showdocument('#DocumentNo#','ZoomIn')">					
				
				<!--- end of 30Jul04 modification --->				
				</td>	
				<td class="labelit">#DocumentNo#</td>
				<td class="labelit">#Mission#</td>
				<td class="labelit">#Description#</td>
	    		<td class="labelit">#PersonCategory#</td>		
				<td class="labelit">#DateFormat(RequestDate,client.dateformatshow)#</td>
			    <td class="labelit">#Dateformat(PlannedDeployment, "#CLIENT.dateformatshow#")#</td>	
				<!--- added 19Jun04 - if request submitted within 15 days of deployment date, show alert icon --->
				<td align="left">
				...
				<!--- end of 19Jun04 add'l code --->
	    		<td class="labelit"></td>				
	    		<td class="labelit">#PersonCount#</td>
		    	<td class="labelit">#DutyLength#</td>
		    	<td class="labelit">#TaNumber#</td>				
    			<td class="labelit">#OfficerUserFirstName# #OfficerUserLastName#</td>
				<!---
		...
		<cfif Status EQ "0" AND URL.IDSorting NEQ "ActionDescription">
			...
			<tr bgcolor="#IIf(CurrentRow Mod 2, DE('ffffff'), DE('EBEBEB'))#">		
				<td>&nbsp;</td>		
				<td colspan="4" align="left" class="labelit">
				  <cfif ActionArea eq "TU">
					<cfif ActionDescription EQ "Review travel request">
						<font color="FF00FF">
					<cfelseif ActionDescription EQ "Initiate travel authorization">
				...
				<td colspan="9" class="labelit" align="left">
				<cfif sRemarks NEQ "">
			...
			<cfelse>
				<tr bgcolor="#IIf(CurrentRow Mod 2, DE('ffffff'), DE('EBEBEB'))#">		
				  ...
				  <cfif URL.IDSorting EQ "ActionDescription">
					<cfif sRemarks EQ "">
						<td colspan="4" align="left" class="labelit">&nbsp;</td>
					<cfelse>
						<td colspan="4" align="left" class="labelit">&nbsp;</td>
					</cfif>
					    <td colspan="9" class="labelit" align="left">TU: #sRemarksTvl#</td>					
				  <cfelse>
					    <td colspan="5" align="left" class="labelit">&nbsp;</td>
						<td colspan="8" class="labelit" align="left">TU: #sRemarksTvl#</td>
				  </cfif>
		   		</tr>
		</cfif>
    </cfif>		
	...
	<cfif URL.IDLayout EQ "Detail">	
		...
		<cfquery name="GetCandidateDate" 
		   datasource="#CLIENT.Datasource#" username="#SESSION.login#" password="#SESSION.dbpw#">
		...
		<cfif #GetCandidateDate.RecordCount# GT 0>
			<tr bgcolor="#IIf(DocCurrentRow Mod 2, DE('ffffff'), DE('EBEBEB'))#">
				<td colspan="13">
 				<cfinclude template="DocumentExpDeployment.cfm"> 
				</td>
			</tr>
		</cfif>
	</cfif>
