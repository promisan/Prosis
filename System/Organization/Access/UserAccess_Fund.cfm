...
-->
	...
	<cfif tree eq "1">
	 ...
	 <cfquery name="AccessList" 
		...
		FROM    Ref_AllotmentEdition R LEFT OUTER JOIN userQuery.dbo.#SESSION.acc#TreeAccess A 
				ON R.EditionId = A.ClassParameter
	...
	<cfelse>
		...
		username="#SESSION.login#" 
		password="#SESSION.dbpw#">
		SELECT DISTINCT R.EditionId as Code, R.Description, 
		        A.ClassParameter, 
		...
		FROM    Ref_AllotmentEdition R LEFT OUTER JOIN Organization.dbo.OrganizationAuthorization A
		  ...
		  AND     A.Role        = '#URL.ID#'
		GROUP BY R.EditionId, A.ClassParameter
		</cfquery>
	...
	<tr  class="labelmedium line">
	  <td style="padding-left:4px">#Code# #Description# <cfif period neq "">(#Period#)<cfelse>(all)</cfif></td>
