...
<cfsavecontent variable="insertStatement">
	...
	<cfoutput query="getPortal">
     ...
     VALUES
           ...
           ,'#replace(FunctionClass,"'","''","ALL")#' 
           ,'#replace(FunctionName,"'","''","ALL")#' 
           ...
           <cfif FunctionDirectory neq "">,'#replace(FunctionDirectory,"'","''","ALL")#' </cfif>
...
<cfsavecontent variable="updatePortal">
	...
	#vBL##vBL##vBL#
	--Clear previous configuration for "#ucase(url.name)#" Portal
	#vBL#
	#deleteStatement# 
	
	#vBL##vBL##vBL#
	
	--Insert new Portal configuration for "#ucase(url.name)#" Portal
	#vBL#
	#insertStatement# 
