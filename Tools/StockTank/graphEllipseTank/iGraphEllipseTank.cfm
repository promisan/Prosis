...
<script>	
	...
	function draw_ellipseTank()
	{	
			var container = document.getElementById('graphcontainer');
			var x = document.getElementById('x').value;
			var y = document.getElementById('y').value;
			var rad = document.getElementById('radius').value;
			var rad2 = document.getElementById('radius2').value;
			...
			var vbrowser = '<cfoutput>#client.browser#</cfoutput>';
			var movedown = pipesize;
			var moveright = 8*borderwidth;
			...
			vAttributes["fill"] = "r(0.25,0.25) - #" + lighterColor1 + " - #" + col1;
			if (vbrowser != "Explorer") {
			...
			var clipY = 2*(rad*(1 - fillPercentage)) + parseInt(borderwidth);
			var clipWidth = 4*rad2 + parseInt(borderwidth) + parseInt(strapwidth);
			var clipHeight = 2*rad + parseInt(borderwidth) + parseInt(strapwidth);
			vAttributes["clip-rect"] = 0 + "," + (clipY + parseInt(movedown)) + "," + clipWidth + "," + clipHeight;
			
			var iFilled = paper.ellipse(parseInt(rad2) + parseInt(borderwidth), parseInt(rad) + parseInt(movedown) + parseInt(borderwidth), rad2, rad).attr(vAttributes);						
			...
			var iPerimeter = paper.ellipse(parseInt(rad2) + parseInt(borderwidth), parseInt(rad) + parseInt(movedown) + parseInt(borderwidth), rad2, rad).attr(vAttributes);
			
			//pipe-in/out			
			var iPipeIn = paper.image(pipeImage, rad2 - pipesize/2 + parseInt(borderwidth), (parseInt(borderwidth)) + 2*parseInt(borderwidth), pipesize, pipesize);
			var iPipeOut = paper.image(pipeImage, rad2 - pipesize/2 + parseInt(borderwidth), (2*parseInt(rad) + parseInt(borderwidth)) - 2*parseInt(borderwidth), pipesize, pipesize);
			
			if (showfillmark == "1")
			{	
				//fillImage						
				var initFillIconX = parseInt(rad2) - iconSize;
				...
				var initFillTextX = initFillIconX - 2;
			...
			if (showsubLevel == "1")
			{
				...
				var initSublevelX = 0;
				var initSublevelY =  2*(rad*(1 - sublevelPercentage)) + parseInt(borderwidth);
				var endSublevelX = 2*rad2 + 2*parseInt(borderwidth);
				...
				var initTextX = 2*rad2 + parseInt(borderwidth);
			...
			if (showMinimumLevel == "1")
			{
				...
				var initMinimumlevelY =  2*(rad*(1 - minimumlevelPercentage)) + parseInt(borderwidth);
				var endMinimumlevelX = 2*rad2 + 2*parseInt(borderwidth);
				...
				var initMTextX = 2*rad2 + parseInt(borderwidth);
			...
			if (showstrap == "1")
			{
				...
				var initStrapX = parseInt(rad2) + parseInt(borderwidth);
				var initStrapY =  parseInt(borderwidth);
				var endStrapY = 2*parseInt(rad) + parseInt(borderwidth);
				...
				var initMarkY = 2*(rad*(1-strapfillpercentage)) + parseInt(borderwidth);
				...
				var initTextX = initMarkX + parseInt(iconSize)*2 - 2;
			...
			if (showsubLevel == "1" && showullage == "1")
			{
				...
				var initUllageX = 2*parseInt(rad2) + 2*parseInt(borderwidth) + 10;
	...
	window.onload = function () {draw_ellipseTank();}
...
<body style="background-color:#url.backgroundcolor#;">
	...
	<input type="hidden" name="x" id="x" value="#url.radius2#">
	<input type="hidden" name="y" id="y" value="#url.radius#">
	<input type="hidden" name="radius" id="radius" value="#url.radius#">
	<input type="hidden" name="radius2" id="radius2" value="#url.radius2#">
	...
	<input type="hidden" name="wWidth" id="wWidth" value="#url.wid#">
	
	<cfif url.showtooltip eq 1>
		<cf_UIToolTip sourcefortooltip="#url.tooltip#">
			<div id="graphcontainer" align="center" style="width:#url.wid#; height:#url.hei#;"></div>
		</cf_UIToolTip>
	<cfelse>
		<div id="graphcontainer" align="center" style="width:#url.wid#; height:#url.hei#;"></div>
	</cfif>
