...
<cfoutput query="GetAction">
		...
		<cfquery name="Action" 
		 ...
		 password="#SESSION.dbpw#">
			...
			<cfelseif Attributes.actiondatemode eq "Planning">
				...
				<td class="labelmedium" valign="top" style="padding-top:7px;padding-left:5px">#Description#:</td>
				...
				<tr>		
					<td style="padding-top:4px;padding-right:18px" class="labelit"><cf_tl id="Requested for">:</td>
					<td style="padding-top:4px;padding-right:4px"  class="labelit"><cf_tl id="Scheduled for">:</td>				
				</tr>
				...
				<tr>
					...
					<td style="font-size:15px;padding-left:10px;padding-right:10px">													
						...
						<cfelse>
							...
							<select class="regularxl" name="PositionNo#Code#" id="PositionNo#Code#">
					...
					<td class="labelmedium" style="font-size:15px;padding-right:6px">
						...
						<cfelse>	
							...
							<cfif attributes.time eq "Yes">
								...
								<cfdiv id="_woaf_datetime" bind="url:#session.root#/tools/process/workOrder/WorkOrderActionFields_Datetime.cfm?code=#code#&val=#stDT#&positionNo={PositionNo#Code#}">
							
							<cfelseif attributes.calendar eq "8">	
						
								<cf_intelliCalendarDate8
									FieldName="DatePlanning#Code#" 
									Default="#st#"
									class="regularxl enterastab"
									AllowBlank="False">	
					...
					<td style="font-size:15px;padding-left:4px">
					
						<select class="regularxl" name="PlanOrderCode#Code#">
					...
					<td valign="top" style="padding-top:4px;font-size:15px;padding-right:10px;padding-right:10px">
						...
						<cfif ActionFulfillment eq "WorkPlan" and attributes.mode eq "Edit">				
							...
							<td valign="top" style="padding-left:10px">
								...
								<cfdiv id="workplan" 
								  bind="url:#session.root#/WorkOrder/Application/Delivery/Planner/WorkPlan.cfm?action=embed&workactionid=#action.workactionid#&dts={DatePlanning#Code#}&mission=#url.mission#">					
						...
						<cfelseif ActionFulfillment eq "Schedule" and attributes.mode eq "Edit">
							...
							<cfoutput>
							<input type="button" name="Schedule" value="#qSchedule#">
							</cfoutput>
	...
	<cfelse>
	    ...
	    <cfparam name="FORM.DateMemo#Code#" default="">		
		<cfset memo  = Evaluate("FORM.DateMemo#Code#")>
		
		<!--- -------------------------------------------------------------------------------------------------- --->
		<!--- Requested date ----------------------------------------------------------------------------------- --->
		<!--- -------------------------------------------------------------------------------------------------- --->
		
		<cfparam name="FORM.DateRequested#Code#" default="">	
		<cfset dateValue = "">
		<cfset rdte   = Evaluate("FORM.DateRequested#Code#")>	
		...
		<cfif check.recordcount eq "0" or check.DateTimePlanning eq "">
			...
			<cfif rdte neq "">				
				<cfparam name="FORM.DatePlanning#Code#" default="#dateformat(rdte,client.dateformatshow)#">	
			<cfelse>
				<cfparam name="FORM.DatePlanning#Code#" default="#dateformat(now(),client.dateformatshow)#">	
			</cfif>			
				
		<cfelse>
		
			<!--- we get the existing planning date --->						
			<cfparam name="FORM.DatePlanning#Code#" default="#dateformat(check.DateTimePlanning,client.dateformatshow)#">	
		...
		<cftry>		
			...
			<cfset pdte   = Evaluate("FORM.DatePlanning#Code#_date")>
			<CF_DateConvert Value="#pdte#">
			<cfset pdte = datevalue>					
			<cfset hour  = Evaluate("FORM.DatePlanning#Code#_hour")>
			<cfset pdte  = DateAdd("h",  hour,  pdte)>
			<cfset minu  = Evaluate("FORM.DatePlanning#Code#_minute")>
			<cfset pdte  = DateAdd("n",  minu,  pdte)>
			
			<cfparam name="FORM.PositionNo#Code#" default="">				
			<cfset pos  = Evaluate("FORM.PositionNo#Code#")>
			
			<cfparam name="FORM.PlanOrderCode#Code#" default="">				
			<cfset pln  = Evaluate("FORM.PlanOrderCode#Code#")>
			...
			<cfcatch>
				...
				<cfset pdte   = Evaluate("FORM.DatePlanning#Code#")>
				<CF_DateConvert Value="#pdte#">
				<cfset pdte = datevalue>					
		...
		<cfparam name="FORM.DateActual#Code#" default="">	
		<cfset adte   = Evaluate("FORM.DateActual#Code#")>
		<cfset dateValue = "">
		<cfif adte neq "">
