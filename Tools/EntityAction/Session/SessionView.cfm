...
<cfquery name="Document" 
 ...
 password="#SESSION.dbpw#">
 	...
 	FROM      Ref_EntityDocument AS R LEFT OUTER JOIN
              Ref_EntityActionPublishDocument AS D ON R.DocumentId = D.DocumentId 
			                                 AND D.ActionCode    = '#Object.ActionCode#' 
											 AND ActionPublishNo = '#Object.ActionPublishNo#'
	WHERE     R.DocumentType = 'session'
</cfquery>
...
<cfif Object.recordcount eq "1">
	...
	<cfform method="POST" name="sessionform" onsubmit="return false">
		...
		<table width="94%" class="formspacing formpadding" align="center">
			...
			<tr class="labelmedium"><td><cf_tl id="Web form"></td>
			...
			<tr>
				...
				<td id="link"><a href="#session.root#/ActionSession.cfm?id=#session.actionSessionId#&mode=test" target="_blank"><cf_tl id="Test link"></a></td>			
			</tr>
			...
			<cfelse>
				...
				<tr class="labelmedium"><td><cf_tl id="IP used"></td>
				    <td><input type="text" class="regularxxl"  name="SessionIP" value="#session.sessionIP#"></td>
				</tr>
				
				<tr class="labelmedium"><td><cf_tl id="Interaction"></td>
				    <td style="font-size:17px">#dateformat(Session.SessionActualStart,client.dateformatshow)# #timeformat(Session.SessionactualStart,"HH:MM:SS")#
			...
			<td colspan="2">
				...
				<td><input type="button" class="button10g" type="text" name="Save" value="Save" onclick="workflowsessionsave('#url.actionid#','#url.referenceid#')"></td>
				</tr>
				</table>
			</td>
			</tr>		
