...
(function( window, $ ){
	...
	function PhotoTagger( container, settings ){
		...
		this.tag_close = $( "<div class='" + this.getFullClassName( "close" ) + "'></div>" );
		...
		this.container.mousedown(
			function( event ){
				...
				if (
					...
					!target.is( "div." + self.getFullClassName( "tag" ) )
					){
						...
						{
							var tagPosition = self._CURRENT_TAG.position();
							_left = tagPosition.left+self._CURRENT_TAG.outerWidth()-21;
							_top  = tagPosition.top+6;
							
							 
							if (event.clientX>=_left && event.clientX<=_left+10)
							{
								if (event.clientY>=_top && event.clientY<=_top+10)
								{
									if (confirm( "Delete this tag?" )){
										// Delete the tag.
										
										self.deleteTag( self._CURRENT_TAG );
									}								
									return;
								}
							}
	...
	PhotoTagger.prototype = {
		...
		// I add a tag to the 
		addTag: function( id, x, y, width, height, message, value ){
			var self = this;
		...
		createTag: function( left, top, width, height ){
			// Create the new tag.
			var tag = $( "<div class='" + this.getFullClassName( "tag" ) + "'><br /></a>" );
		...
		emphasizeTag: function( tag ){
			...
			this.tag_close
				.css({
					left: ((tagPosition.left+tag.outerWidth()-21) + "px"),
					top: ((tagPosition.top+6) + "px")
				}).show();
		...
		saveTagRecord: function( id, x, y, width, height, message, value, photoID, onSuccess ){
			var self = this;
		...
		teardownPendingTagCreation: function(){
			...
			if (this.isPendingTagSizeValid()){
				...
				var tagPosition = this.pendingTag.position();
				
				
				$("#dselection").css({
								left: (tagPosition.left + "px"),
								top: ((tagPosition.top + this.pendingTag.outerHeight()) + "px")
							}).show();
							
				var _ms = $("#multiselect").data("kendoMultiSelect");
				_ms.value("");							
							
				_current = this.container;								
	...
	$.fn.photoTagger.defaultOptions = {
		...
		css: {
			"div.tag": {
				"background-image": "url( vRoot+'/tools/tagger/transparent.gif' )",
			...
			"div.close": {
				"background-image": "url( vRoot+'/images/close3.gif' )",
