...
<cfset vThiSummaryComplementRegionGeographical = evaluate("session.summaryComplementRegionGeographical_#url.viewId#")>
<cfset vThisummaryComplementRegionGrades = evaluate("session.summaryComplementRegionGrades_#url.viewId#")>
    ...
    SELECT  CountryGroup 
            <cfoutput query="getPeriods">
                , (
                    ...
                    <cfif vThisummaryComplementRegionGrades eq "1" OR vThisummaryComplementRegionGrades eq "yes" OR vThisummaryComplementRegionGrades eq "true">
                    AND     (LEFT(Gx.PositionGrade,1) IN ('P','D') OR Gx.PositionGrade IN ('ASG','USG'))
                    </cfif>
                    <cfif vThiSummaryComplementRegionGeographical eq "1" OR vThiSummaryComplementRegionGeographical eq "yes" OR vThiSummaryComplementRegionGeographical eq "true">
...
<cfoutput>
    <h3 style="padding-bottom:10px;">
        <img src="#session.root#/images/Excel.png" style="cursor:pointer;" width="30" height="30" onclick="Prosis.exportToExcel('detailGeoCompLongitudinalByRegionContent#url.viewId#Detail');" title="#lblExportToExcel#">
        LONGITUDINAL BY REGIONAL GROUP
    </h3>
</cfoutput>

<table class="table tableDetail table-striped table-bordered table-hover detailGeoCompLongitudinalByRegionContent<cfoutput>#url.viewId#</cfoutput>Detail" id="detailGeoCompLongitudinalByRegionContent<cfoutput>#url.viewId#</cfoutput>Detail" style="width:100%;">
    <thead>
        <tr>
...
<cfset ajaxOnLoad("function(){ $('.detailGeoCompLongitudinalByRegionContent#url.viewId#Detail').DataTable({ 'pageLength':25, 'lengthMenu':[10,25,50,100,200], 'order': [[0, 'asc']] }); }")>

<!--- Line chart --->
<cfset "doChart_summaryComplementLongitudinalByRegionMapChart" = "">

<cfset vSeriesCount = 1>
<cfset vSeriesArray = ArrayNew(1)>

<cfquery name="getComplementDataDetailDistinctGroups" dbtype="query">
    SELECT  DISTINCT CountryGroup
    FROM    getComplementDataDetail
</cfquery>
...
<cfoutput query="getComplementDataDetailDistinctGroups">
    ...
    <cfquery name="getComplementDataDetailGraph" dbtype="query">
        ...
        <cfloop query="getPeriods">
            ...
            FROM    getComplementDataDetail
            WHERE   CountryGroup = '#getComplementDataDetailDistinctGroups.CountryGroup#'
    ...
    <cfset vSeries.name = "#CountryGroup#">
...
<cf_mobileGraph
    id = "summaryComplementLongitudinalByRegionMapChart"
    type = "Line"
    series = "#vSeriesArray#"
    responsive = "yes"
    scaleStep = "1"
...
<cfset ajaxOnLoad("function(){ #doChart_summaryComplementLongitudinalByRegionMapChart# }")>

<cfoutput>
    <style>
        .detailGeoCompLongitudinalByRegionContent#url.viewId#Detail thead tr th {
            font-size:12px;
        }
    </style>
</cfoutput>
