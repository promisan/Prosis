...
//localDragFunc()
//window.status = 'dd.elements.' + dd.obj.name + '.x  = ' + dd.obj.x + '	 dd.elements.' + dd.obj.name + '.y = ' + dd.obj.y + '  scroll element: '+ dd.getScrollY();
...
/* THIS ONE IS CALLED ONCE AN ITEM IS DROPPED BUT NEEDS TO BE RETURNED
Added by promisan bv. April 2007, this method is called from the custom side after a drag is completed
from the localDrag method as follows :
moveActionStep(dd.obj.name, stepx, stepy, iter)
*/

function returnDrag(objName, stepx, stepy, iter) {

	dobj = dd.elements[objName]

	if (iter > 0) {
	
		movex = dobj.x - stepx
		movey = dobj.y - stepy
		dobj.moveTo(movex, movey)
		
		iter--			
		setTimeout("returnDrag('"+objName+"',"+stepx+","+stepy+","+iter+")",1)
