					...
					<cfif structKeyExists(Error, "RootCause")>
						<cfoutput>
							<cfif structKeyExists(Error.RootCause, "SQL")>
								...
								<cfdump var="#error.RootCause.SQL#">
							</cfif>
							<cfif structKeyExists(Error.RootCause, "Message")>
									</br></br>
								<cfdump var="#error.RootCause.Message#">
							</cfif>
							<cfif structKeyExists(Error, "Diagnostics")>
									</br></br>
								<cfdump var="#error.Diagnostics#">
							</cfif>
							<cfif structKeyExists(Error.RootCause, "Cause")>
								<cfif structKeyExists(Error.RootCause.Cause, "Type")>
										</br></br>
									<cfdump var="#error.RootCause.Cause.Type#">
								</cfif>

								<cfif structKeyExists(Error.RootCause.Cause, "TagContext")>
										</br>
									<cfloop array="#error.RootCause.Cause.TagContext#" index="idx">
											&nbsp;&nbsp;&nbsp;#idx.template# #idx.line#
											</br>
									</cfloop>
								</cfif>

								<cfif structKeyExists(Error.RootCause.Cause, "StackTrace")>
										</br>
									#paragraphFormat2(error.RootCause.Cause.StackTrace)#
								</cfif>
