...
<!--- check if this object has a workflow --->
			...
			<cfif TransactionSource eq "AccountSeries" or TransactionSource eq "ReconcileSeries">
<!--- always a workflow --->
				...
				<cfset cnt = 0>
					...
					<cfif ParentJournal neq "">
							...
							<cf_tl id="Subsequent transaction">:</font>
								<a href="javascript:ShowTransaction('#journal#','#journalserialNo#','1')"><font color="0080C0"><cfif org.orgunitname neq "">#Org.OrgunitName#:</cfif> #Jrn.Description# #JournalTransactionNo# </a>
						</td>
...
<!--- retrieve related lines : not clear at this point which is the information shown : asset lines ???? --->
							...
							<cfquery name="Link"
									...
									FROM    TransactionHeader H,
									TransactionLine T, Ref_Account R
