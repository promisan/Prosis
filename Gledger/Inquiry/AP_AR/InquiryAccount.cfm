	...
	FROM (
		...
		FROM TransactionHeader H, 
		          ...
		          (SELECT        CASE WHEN L.ParentJournal IS NULL THEN L.Journal ELSE L.ParentJournal END AS Journal, 
                   ...
                   WHERE     L.GLAccount IN ('#url.account#') 
		...
		AS derivedtbl_1, Journal J
		
		WHERE derivedtbl_1.Journal = H.Journal 
		AND   derivedtbl_1.JournalSerialNo = H.JournalSerialNo 
		AND   derivedtbl_1.Amount <> 0 
...
<cfset fields[itm] = {label   = "#vAmount#", 					
					...
					align   = "right",
					aggregate  = "sum", 
					formatted  = "numberformat(AmountBase,'__,__.__')",
					search  = "number"}>	
