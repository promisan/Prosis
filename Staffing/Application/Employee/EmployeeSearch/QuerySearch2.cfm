...
<cfquery name="SelectA" 
		...
		FROM PersonSearchLine
		WHERE SearchId = '#URL.ID#'
		AND SearchClass = 'SalaryGroupOperator'
</cfquery>

<cfif #Select.RecordCount# gt 0>
   <cfset LastFrom  = #LastFrom#&", tmp#SESSION.acc#SCHSelect">
   <cfset LastWhere = "WHERE tmp#SESSION.acc#SCHSelect.PersonNo = tmp#SESSION.acc#PersonAll.PersonNo">
   <!--- run queries --->
   <CF_QuerySearchSCH
   FieldValue="#URL.ID#"
...
<cfquery name="SelectA" 
		...
		FROM PersonSearchLine
		WHERE SearchId = '#URL.ID#'
		AND SearchClass = 'TriggerOperator'
</cfquery>

<cfif #SelectTRG.RecordCount# gt 0>
   <cfif #LastWhere# neq "">
   <cfset LastFrom  = #LastFrom#&", tmp#SESSION.acc#PTESelect">
   <cfset LastWhere = #LastWhere#&" AND tmp#SESSION.acc#TRGSelect.PersonNo = tmp#SESSION.acc#PersonAll.PersonNo">
   <cfelse>
   <cfset LastFrom  = #LastFrom#&", tmp#SESSION.acc#TRGSelect">
   <cfset LastWhere = " WHERE tmp#SESSION.acc#TRGSelect.PersonNo = tmp#SESSION.acc#PersonAll.PersonNo">
   </cfif>   
   <!-- run queries --->
   <CF_QuerySearchTRG
   FieldValue="#URL.ID#"
...
window.open("ResultView.cfm?ID=#URL.ID#&Mission=#URL.Mission#&IDForm=Search2.cfm", "_top"); 
