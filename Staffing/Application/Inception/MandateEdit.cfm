...
<cfoutput>
	...
	<script>
		...
		function recopy() {				   
		   ProsisUI.createWindow('mydialog', 'Copy again the staffing information', '',{x:100,y:100,height:650,width:980,modal:true,resizable:false,center:true})    						
		   ptoken.navigate('RecopyAssignmentDialog.cfm?mission=#URL.ID#&mandateparent=#GetParent.MandateNo#&mandateNo=#URL.ID1#','mydialog')  			
		}
		
		function extend() {	   
			ptoken.navigate('RecopyAssignmentGo.cfm?mission=#URL.ID#&MandateParent=#GetParent.MandateNo#&MandateNo=#URL.ID1#&onlynew='+document.getElementById('onlynew').checked+'&extend='+document.getElementById('extendfirst').checked,'processbox')
		}
		
		function transform(docno) {
		 	ptoken.open('TransformView/TransformView.cfm?ID=#URL.ID#&ID1=#URL.ID1#&ID4=' + docno,  'mandatescreen', 'left=20, top=20, width=' + w + ', height= ' + h + ', toolbar=no, status=yes, scrollbars=yes, resizable=no');
		}		
		...
		function mandate() {		    
			...
			ptoken.navigate('MandateView.cfm?ID=#URL.ID#&ID1=#URL.ID1#','mydialog')  
		}
		
		function PrintCustom(ad) {
			ptoken.open("#SESSION.root#/#GetParameter.PersonActionTemplate#?ID=#URL.ID#&ID1=#URL.ID1#&ID2="+ad)
		}
...
<cfform action="MandateEditSubmit.cfm" method="POST" name="mandateform">
	...
	<table width="97%" align="center" class="formpadding">
	  ...
	  <tr class="line labelmedium2"><td colspan="2" style="font-weight:200;height:50px;font-size:28px">Maintain Staffing Period #Get.MandateNo#</td></tr>   
	  ...
	  <tr>
		...
		<cfif getParent.Description neq "">
			...
			<td style="padding-left:3px"><cf_tl id="Parent Period">:</td>
			...
			<cfif orgAccess eq "READ" or orgAccess eq "EDIT" or orgAccess eq "ALL"> 
				...
				<td>
					<table>
						...
						You may reinitialize this staffing period by carrying over the ASSIGNMENTS that were valid by the end of the prior (parent)	mandate.
						</td></tr>
						<tr><td id="boxcopy">
			 ...
			 <table>
			  ...
			  <td id="status">
				  ...
				  <td>
					  ...
					  <br>
					  <a href="javascript:ptoken.navigate('MandateOpen.cfm?mission=#url.id#&mandateno=#url.id1#','status')">Click here to [Unlock Staffing period]</a>
					  </cfif>		  
				  
				  </td>
				  <td style="padding-left:30px"></td>
				  <td><b><font color="FF0000">Attention:</font></b><br>Unlocking a mandate should only be done if you know what you are doing!</font></td>
				  </tr>			  
				  </table>
				 </cfif> 
			  </td>		  
				...
				<tr><td>
					...
					<cfoutput>
						...
						<cfloop index="itm" from="1" to="20" step="1">
							...
							<cfif itm Mod 2><tr class="labelmedium2"></cfif>
											
								<td style="padding-left:6px;padding-right:3px">#itm#:</td>
								<td>
								...
								<td style="padding-left:3px;padding-right:4px">Nme:</td>
