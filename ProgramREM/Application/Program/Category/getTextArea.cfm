...
password="#SESSION.dbpw#">
	...
	FROM    ProgramCategoryStatus S 
			RIGHT OUTER JOIN Ref_ProgramStatus F 
				ON S.ProgramStatus = F.Code 
				AND S.ProgramCode = '#URL.ProgramCode#'
				AND S.ProgramCategory = '#URL.Code#'
...
<cfif url.mode eq "fly">
	<cfset sc = "categorysave('1','#url.programcode#','#url.code#','#url.period#')">
<cfelse>
	<cfset sc = "">	
</cfif>
			...
			<select name="Status#url.code#_#StatusClass#" style="width:200px" class="regularxl enterastab" 
			   onchange="#sc#">						
				 <cfloop query="StatusSelect">
				  <option value="#Code#" <cfif Selected eq Code> selected</cfif>>(#Weight#) #Description#</option>
				 </cfloop>
			</select>
		</cfoutput>
...
<cfquery name="getCodes" 
   ...
   password="#SESSION.dbpw#">
		SELECT *
	    FROM   Ref_ProgramCategoryProfile R, 
		       Ref_TextArea T
