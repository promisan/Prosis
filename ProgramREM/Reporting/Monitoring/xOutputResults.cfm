...
<cfif #SelectSubPeriod.recordCount# eq 0>
	<cfset ProgressWhere = "">
<cfelse>
   <cfset ProgressWhere = "AND PO.ActivityPeriodSub = '#SelectSubPeriod.SelectId#' AND (PO.RecordStatus != 9 or PO.RecordStatus is NULL)">
</cfif>   
...
<cfif #SelectStatus.recordCount# neq 0>
	<cfif #ProgressWhere# eq "">
		<cfset ProgressWhere = "AND PP.ProgressStatus = '#SelectStatus.SelectId#'">
	<cfelse>
...
username="#SESSION.login#" 
password="#SESSION.dbpw#">
Select PA.ActivityId, PO.ActivityOutput, PO.ActivityOutputDate, PA.Reference, PA.ActivityDescription, R.DescriptionShort, 
			PP.ProgressStatus, PP.ProgressStatusDate, Rs.Description As ProgressDescription, Pp.Created
	FROM Program P, ProgramActivityOutput PO, ProgramActivity PA, Ref_SubPeriod R, ProgramActivityProgress PP, Ref_Status Rs
	...
	AND PO.OutputID = PP.OutputID 
	AND (PP.RecordStatus != 9 or PP.RecordStatus is NULL)
	AND Rs.ClassStatus = 'Progress'
	AND Rs.Status = PP.ProgressStatus 
...
<cfset CurrentActivity = #Output.ActivityOutput#>
