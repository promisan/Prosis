...
<cfif #SelectProgressDate.recordCount# neq 0 AND #SelectProgressDate.SelectID# neq "ANY">
 ...
 <cfset ProgressWhere = "AND Pp.Created BETWEEN #StartDTE# AND #EndDTE# ">
...
<CF_DropTable dbName="AppsQuery" tblName="tmp#SESSION.acc#ProgressAll">	
  ...
  <cfquery name="ResultProgress" 
    ...
    username="#SESSION.login#" 
    password="#SESSION.dbpw#">
    SELECT DISTINCT Pe.ProgramId, Pe.ProgramCode, Pe.OrgUnit, Pp.OfficerUserId, Pp.ProgressId
    INTO  userQuery.dbo.tmp#SESSION.acc#ProgressAll
    FROM  ProgramPeriod Pe, ProgramActivity Pa, ProgramActivityOutput Po, ProgramActivityProgress Pp
	...
	AND PA.ActivityPeriod = Pe.Period
...
<cfquery name="Result" 
	  ...
	  INTO UserQuery.dbo.tmp#SESSION.acc#Final
	  FROM   UserQuery.dbo.tmp#SESSION.acc#ProgramAll
			 #LastFrom#
	    #LastWhere#
</cfquery>
...
<cfquery name="Result" 
       ...
       username="#SESSION.login#" 
       password="#SESSION.dbpw#">
	   INSERT INTO ProgramSearchResult(SearchId, ProgramId)
