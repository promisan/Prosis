...
<cfinclude template="../../Template/IndicatorTarget.cfm">
  ...
  <cfquery name="Base" 
	...
	SELECT    DISTINCT S.*, D.EPasRatingDescription
	FROM      EmployeeSnapShot.dbo.HRPO_AppStaffingDetail S, 
	          EmployeeSnapShot.dbo.HRPO_EPasDetail D
	WHERE     S.IndexNo = D.IndexNo
	AND       S.Mission        = '#Target.OrgUnitCode#' 
	AND       S.SelectionDate  = '#Audit.AuditDate#' 
	AND       S.PostType       = 'International'
	AND       S.PersonNo is not NULL
	...
	AND  ePASPlanFROSignedDate <= '#Audit.AuditDate#'	 		
 </cfquery>	
...
<cfoutput query="List" group="HierarchyCode">
	...
	<cfoutput>
		...
		<tr bgcolor="#cl#">
			...
			<td>#FullName#</td>
			<td>#ePasRatingDescription#</td>
			
		</tr>
		<tr><td colspan="6" bgcolor="d0d0d0"></td></tr>
