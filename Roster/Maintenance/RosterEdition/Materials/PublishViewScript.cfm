...
function doSend(edition) {	
	...
	if (id!='' && sp!='')
		window.open("#SESSION.root#/Tools/Mail/Broadcast/BroadCastEdition.cfm?submissionedition="+edition+"&readonly=yes&sourcepath="+sp,"broadcast","status=no,height=850px,width=920px,scrollbars=no,toolbar=no,resizable=no");		
}
...
function doPrepare(edition,actionid) {		
	...
	ColdFusion.ProgressBar.start('pBar') ;
 	ColdFusion.navigate('#SESSION.root#/Roster/Maintenance/RosterEdition/Materials/prepareDocument.cfm?submissionedition='+edition+'&actionid='+actionid, 'setdocument');
}
