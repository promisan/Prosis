...
function quesave(topic) {
   ...
   document.mytopic.onsubmit() 
	if( _CF_error_messages.length == 0 ) {
       ColdFusion.navigate('../Bucket/BucketQuestion/RecordListingSubmit.cfm?idfunction=#url.idfunction#&topicid='+topic,'listing','','','POST','mytopic')
	 }   
 }
...
function candidate(id) {
	...
	ColdFusion.Window.create('CandidateSearchWindow', 'Retrieve a candidate', '',{x:100,y:100,height:document.body.clientHeight-80,width:document.body.clientWidth-80,modal:true,center:true,resizable:false});
	ColdFusion.Window.show('CandidateSearchWindow'); 			
	ColdFusion.navigate('#SESSION.root#/Roster/Candidate/Events/LocateCandidate.cfm?mode=bucket&ID='+id + '&ts='+new Date().getTime(),'CandidateSearchWindow');
...
function memoshow(memo,act,app,fun,row) {
	...
	if (se.className == "hide") {
		...
		ColdFusion.navigate(url,'i'+memo)	
...
function memosave(memo,app,fun) {
	...
	ColdFusion.navigate(url,'p'+memo)	
